...
// Format chunk optimisé : tableau dense d'IDs, position déduite du chunk (cx,cz) + index
const CHUNK_Y_MIN = -16;
const CHUNK_Y_MAX = 127;
const CHUNK_LAYERS = CHUNK_Y_MAX - CHUNK_Y_MIN + 1;
const CHUNK_VOLUME = 16 * 16 * CHUNK_LAYERS;
const MOB_SAVE_RADIUS = 80; // blocs : sauvegarder les mobs dans ce rayon du joueur

function chunkIndex(lx, ly, lz) {
    return (ly - CHUNK_Y_MIN) * 256 + lz * 16 + lx;
}
function chunkIndexToPos(index) {
    var rest = index % 256;
    var ly = CHUNK_Y_MIN + Math.floor(index / 256);
    var lz = Math.floor(rest / 16);
    var lx = rest % 16;
...
function saveWorldToStorage(worldName) {
    ...
    const getBlockId = (typeof window.getBlockId === 'function') ? window.getBlockId : null;
    const chunkToBlocks = {};
    ...
    var savedMobs = [];
    if (typeof mobs !== 'undefined' && Array.isArray(mobs)) {
        for (var m = 0; m < mobs.length; m++) {
            var mob = mobs[m];
            var dx = (mob.x || mob.group.position.x) - px;
            var dz = (mob.z || mob.group.position.z) - pz;
            if (dx * dx + dz * dz <= MOB_SAVE_RADIUS * MOB_SAVE_RADIUS) {
                savedMobs.push({
                    type: mob.type,
                    x: mob.x != null ? mob.x : mob.group.position.x,
                    y: mob.y != null ? mob.y : mob.group.position.y,
                    z: mob.z != null ? mob.z : mob.group.position.z,
                    hp: mob.hp != null ? mob.hp : 10
                });
            }
        }
    }
    ...
    function serializeChunk(ck) {
        var raw = chunkToBlocks[ck];
        if (!raw) return null;
        if (getBlockId && SAVE_VERSION >= 6) {
            var parts = ck.split('_');
            var cx = parseInt(parts[0], 10);
            var cz = parseInt(parts[1], 10);
            var arr = new Array(CHUNK_VOLUME);
            for (var i = 0; i < CHUNK_VOLUME; i++) arr[i] = 0;
            for (var key in raw) {
                var co = key.split(',').map(Number);
                var lx = co[0] - cx * CHUNK_SIZE;
                var ly = co[1];
                var lz = co[2] - cz * CHUNK_SIZE;
                if (lx >= 0 && lx < 16 && lz >= 0 && lz < 16 && ly >= CHUNK_Y_MIN && ly <= CHUNK_Y_MAX)
                    arr[chunkIndex(lx, ly, lz)] = getBlockId(raw[key]);
            }
            return arr;
        }
        return raw;
    ...
    if (!useIndexedDB()) {
        try {
            ...
            for (let i = 0; i < chunkKeys.length; i++) {
                ...
                const payload = JSON.stringify(serializeChunk(ck));
    ...
    return idbSet('meta_' + name, meta)
        .then(function() {
            const promises = chunkKeys.map(function(ck) { return idbSet('chunk_' + name + '_' + ck, serializeChunk(ck)); });
...
function loadWorldFromStorage(worldName) {
    ...
    function applyData(data) {
        ...
        const getBlockType = (typeof window.getBlockType === 'function') ? window.getBlockType : null;
        if (data.chunkKeys && data.chunkKeys.length > 0 && data._chunks) {
            for (let i = 0; i < data.chunkKeys.length; i++) {
                ...
                if (Array.isArray(chunkBlocksData)) {
                    for (var idx = 0; idx < chunkBlocksData.length; idx++) {
                        var id = chunkBlocksData[idx];
                        if (!id || !getBlockType) continue;
                        var type = getBlockType(id);
                        if (!type) continue;
                        var pos = chunkIndexToPos(idx);
                        addBlock(cx * chunkSize + pos.lx, pos.ly, cz * chunkSize + pos.lz, type);
                    }
                } else {
        ...
        if (data.savedMobs && Array.isArray(data.savedMobs) && typeof spawnMob === 'function') {
            for (var mi = 0; mi < data.savedMobs.length; mi++) {
                var sm = data.savedMobs[mi];
                spawnMob(sm.x, sm.y - 1, sm.z, sm.type);
                var last = (typeof mobs !== 'undefined' && mobs.length) ? mobs[mobs.length - 1] : null;
                if (last && sm.hp != null) last.hp = Math.max(0, sm.hp);
            }
        }
