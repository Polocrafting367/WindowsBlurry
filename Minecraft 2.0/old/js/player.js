...
function updatePhysics(delta) {
            ...
            const moveSpeed = inWater ? 3.2 : 6.0;
                ...
                for(let by = Math.round(pMinY); by <= Math.round(pMaxY); by++) { 
                    for(let bz = Math.round(pMinZ); bz <= Math.round(pMaxZ); bz++) { 
                        ...
                        if (b && !isFlora(b.type) && b.type !== 'snow_layer') { 
                            const bMinY = by - 0.5;
                            const bMaxY = by + 0.5;
                            if(pMinX < bx + 0.5 && pMaxX > bx - 0.5 && pMinY < bMaxY && pMaxY > bMinY && pMinZ < bz + 0.5 && pMaxZ > bz - 0.5) { 
                                return true; 
                            } 
                        } 
                    } 
        ...
        function respawnPlayer() {
            ...
            try {
                if (typeof gameStarted !== 'undefined' && gameStarted && document.body.requestPointerLock) {
                    var req = document.body.requestPointerLock();
