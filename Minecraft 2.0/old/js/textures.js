...
// Remplace ton ancien "const chunkMaterial = ..." par ceci :
        ...
        function createHeadTexturePixel(base, shade, type) {
            ...
            if (type === 'villager_face' || type === 'zombie_face') {
        ...
        (function() {
            const types = ['pig', 'cow', 'chicken', 'villager', 'zombie', 'creeper', 'goat', 'sheep'];
            ...
            const armArgs = { villager: ['#9B7B4A', '#6B5A3A'], zombie: ['#6A8C5A', '#4A6D40'] };
        ...
        window.CREATIVE_CATEGORIES = {
            'Construction': ['dirt', 'stone', 'cobblestone', 'planks', 'sand', 'snow', 'glass', 'wool'],
            'Nature': ['grass', 'wood', 'leaves', 'flower_red', 'flower_yellow', 'tall_grass', 'cactus'],
        ...
        const MOB_TYPES = ['pig', 'cow', 'chicken', 'villager', 'zombie', 'creeper', 'goat', 'sheep'];
        ...
        window.regenerateMobAtlas = function() {
            const size = MOB_ATLAS_SIZE;
            const cols = MOB_ATLAS_COLS;
            const bodyArgs = { pig: ['#F0B0A0', '#E09080', '#FFD0C8'], cow: ['#B8B8B8', '#787878', '#D8D8D8'], chicken: ['#F5F5DC', '#E8E8C8', '#FFF8DC'], villager: ['#8B6914', '#6B4A0A', '#A08020'], zombie: ['#799C65', '#5A7D4A', '#8BA876'], creeper: ['#3D5C1E', '#2D4A14', '#4A7A28'], goat: ['#F5F5F0', '#D8D8D0', '#E8E8E0'] };
            const faceArgs = { pig: ['#F0B0A0', '#E09080', 'pig_face'], cow: ['#B8B8B8', '#787878', 'cow_face'], chicken: ['#F5F5DC', '#E8D8B0', 'chicken_face'], villager: ['#C4A574', '#A08050', 'villager_face'], zombie: ['#799C65', '#5A7D4A', 'zombie_face'], creeper: ['#3D5C1E', '#2D4A14', 'creeper_face'], goat: ['#F5F5F0', '#D8D8D0', 'goat_face'] };
            const legArgs = { pig: ['#D09070', '#A07050'], cow: ['#909090', '#606060'], chicken: ['#E8D8A0', '#C8B880'], villager: ['#5c4a32', '#3d3528'], zombie: ['#5A6D4A', '#3d4a35'], creeper: ['#2D4A14', '#1a3010'], goat: ['#C8B8A0', '#A09080'] };
            const armArgs = { villager: ['#9B7B4A', '#6B5A3A'], zombie: ['#6A8C5A', '#4A6D40'] };
            var tempBody = {}, tempFace = {}, tempLeg = {}, tempArm = {};
            MOB_TYPES.forEach(function(t) {
                var a = bodyArgs[t]; tempBody[t] = new THREE.MeshLambertMaterial({ map: createMobTexturePixel(a[0], a[1], a[2]) });
                a = faceArgs[t]; tempFace[t] = new THREE.MeshLambertMaterial({ map: createHeadTexturePixel(a[0], a[1], a[2]) });
                a = legArgs[t]; tempLeg[t] = new THREE.MeshLambertMaterial({ map: createMobLegTexturePixel(a[0], a[1]) });
     ...
     function isFlora(type) { return floraTypes.includes(type); }
        window.isFlora = isFlora;
        function isTransparent(type) { return transparentTypes.includes(type); }
        ...
        window.createBreakingBlockGeometry = function(blockType) {
            ...
            let vCount = 0;
            for (let f = 0; f < 6; f++) {
                const [tx, ty] = getTileCoords(blockType, f);
