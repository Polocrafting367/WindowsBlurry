...
let probaennemy = 0.15; // Vitesse initiale
                ...
                for (let j = 0; j < numCols; j++) {
                    ...
                    switch (symbol) {
                        ...
                        case '-': // Route horizontale
                            const roadTextureHorizontal = new THREE.TextureLoader().load('road_horizontal.png');
                            ...
                            break;
                        case '|': // Route verticale
                            const roadTextureVertical = new THREE.TextureLoader().load('road_vertical.png');
                            ...
                            break;
                        case '+': // Croisement de routes
                            const roadTextureCross = new THREE.TextureLoader().load('road_cross.png');
...
function detectCollisions() {
    ...
    for (let i = 0; i < enemies.length; i++) {
        const enemyBoundingBox = new THREE.Box3().setFromObject(enemies[i]);
    ...
    scene.children.forEach((object) => {
        if (object.userData.type === 'building') {
            ...
            if (playerBoundingBox.intersectsBox(buildingBoundingBox)) {
                // Gérer la collision avec le bâtiment ici
                speed = -0;
                breack();
                // Autres actions à effectuer en cas de collision avec un bâtiment
            }
        }
    });
}
...
function accelerate() {
    ...
    if (speed < maxSpeed) {
        // Si la vitesse est très proche de zéro, la mettre à une valeur minimale pour éviter la multiplication par zéro
        if (Math.abs(speed) < 0.01) {
            speed = 0.1; // Définir une vitesse minimale si elle est presque nulle
        }
        speed *= accelerationFactor; // Accélération exponentielle
