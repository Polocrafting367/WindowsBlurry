...
function initApp() {
    ...
    showOneTimeMessage(11, "Mise à jour du code appliquée ! v1.2.16.03");
    migrateOldData(); 
...
if (navigator.serviceWorker.controller) {
    const p = currentSession.profileSnap;
...
let totalLostMs = cutOtMs + cutNightMs; // <--- Vérifiez que cette ligne existe
    let effectiveDuration = rawDuration - totalLostMs;
if (!isOTEnabled && rawOtMs > 0) {
        effectiveDuration -= (rawOtMs - cutOtMs);
    }
    // --- 7. Données visuelles ---
...
return {
        ...
        totalLostMs,      // <--- ET SURTOUT QU'ELLE EST ICI DANS LE RETURN
        surplusOtMs: cutOtMs,
        isOTEnabled
    };
}
...
function saveLogEdit() {
    ...
    // 1. On construit l'objet de base (données brutes)
    const updatedLog = {
        ...oldLog,
        dateStr: newDateStr,
    ...
    // --- LE CHANGEMENT EST ICI ---
    // 2. On calcule les métriques finales (Nuit, HS, Arrondis...) MAINTENANT
    // Cela utilise tes settings actuels (otStep, etc.)
    const finalMetrics = calculateLogMetrics(updatedLog);

    // 3. On stocke le résultat figé dans l'objet
    updatedLog.frozenMetrics = finalMetrics;
    // -----------------------------

    // LOGIQUE DE DÉPLACEMENT (Reste inchangée)
...
navigator.serviceWorker.addEventListener('message', (event) => {
    ...
    if (event.data.type === 'NAV_TO_CHRONO') {
        ...
        setTimeout(() => {
            if (currentSession) { 
                console.log("Relance de la notification après 1s");
                restoreTimerUI(); 
            }
