function ouvrirModalModifierLieu(nomLieuActuel) {
    ...
    parcourirArborescenceSansPreconfiguration(arborescence, arborescenceLieux, '', 0, nomLieuActuel);
...
function parcourirArborescenceSansPreconfiguration(arbre, parent, cheminParent = '', niveau = 0, nomLieuActuel) {
    for (const lieu in arbre) {
        ...
        lieuItem.innerHTML = `
            <div class="place-card level-${niveau}" onclick="toggleNiveau(this, ${niveau})">
                ...
                <button onclick="choisirLieu('${nomLieuPourJS}', '${nomLieuActuel}')" style="margin-left: 10px;">👆</button>
            </div>`;
        ...
        if (Object.keys(arbre[lieu]).length > 0) {
            ...
            parcourirArborescenceSansPreconfiguration(arbre[lieu], sousLieuxList, cheminComplet, niveau + 1, nomLieuActuel);
        }
    }
}
